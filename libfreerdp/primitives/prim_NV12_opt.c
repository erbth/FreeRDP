...
pstatus_t ssse3_NV12ToRGB_8u_P2AC4R(const BYTE **pSrc, int *srcStep,
		BYTE *pDst, int dstStep, const prim_size_t *roi)
{
	int lastRow, lastCol;
	BYTE *UVData,*YData;
	int i,nWidth,nHeight,VaddDst,VaddY,VaddUV;
	...
	VaddUV = srcStep[1] - ((nWidth + 7) & 0xFFF8);
		...
		do
		{
			...
			* each value, because always two pixel "share" the same U- (and V-) data */
			...
			/* then we subtract 128 from each value, so we get D */
				r2 = _mm_set_epi16(128,128,128,128,128,128,128,128);
				r0 = _mm_subs_epi16(r0,r2);
			...
			/* Now we've prepared U-data. Preparing V-data is actually the same, just with other coefficients */
				r5 = _mm_set_epi32(0x80078007,0x80058005,0x80038003,0x80018001);
				r2 = _mm_shuffle_epi8(r3,r5);
...
void primitives_init_NV12_opt(primitives_t *prims)
{
	...
	{
		prims->NV12ToRGB_8u_P2AC4R = ssse3_NV12ToRGB_8u_P2AC4R;
	}
#endif
}
